...
std::vector<SanaeTerminologyCandidate> SanaeProjectManager::GenerateCandidates() const {
        sanae::PhaseTimer timer("sanae/profile/GenerateCandidates");
        struct Count {
                std::string display;
                int occurrences = 0;
        ...
        timer.Mark("sort_and_truncate");
        LOG_D("sanae/profile/GenerateCandidates") << "produced " << result.size() << " candidates";
        return result;
}

std::vector<SanaeReviewIssue> SanaeProjectManager::TerminologyConsistencyIssues() const {
        sanae::ScopedTimerWithCount t("sanae/profile/TerminologyConsistencyIssues",
                                       "total", terminology.size());
        ...
        for (auto const& term : active_terms) {
                ...
                for (auto const& use : current_uses) {
                        ...
                        auto actual_text = use.actual.empty() ? std::string(_("not detected").utf8_str()) : use.actual;
                        detail << agi::format(_("Accepted: %s. In this line: %s."),
                                to_wx(term.russian), to_wx(actual_text));
                        ...
                        if (!current_distribution.empty())
                                detail << " " << agi::format(_("Current episode: %s."),
                                        to_wx(current_distribution));
                        if (!project_distribution.empty())
                                detail << " " << agi::format(_("Project: %s."),
                                        to_wx(project_distribution));
...
void SanaeProjectManager::QueueTerminology(SanaeTerminologyDraft draft) {
        ...
        if (normalized.empty() || draft.russian.empty())
                throw std::invalid_argument("Terminology requires English and Russian text");
        draft.operation = "create";
        draft.term_id.clear();
        draft.base_version = 0;
        // Check uniqueness against existing terminology.
        // Uses term.english_normalized when available (server-populated), falls back
        // to recomputing SanaeNormalizeSource for legacy/local-only data.
        // This is O(T) linear scan — not O(1). The existing data structure is a flat
        // std::vector<SanaeTerminologyEntry> (see sanae_project.h:238). An unordered_map
        // index would make this O(1) average but would need to be maintained alongside
        // the vector. For V1 the linear scan is acceptable (T < 1000 in practice).
        for (auto const& term : terminology)
        ...
        // Check uniqueness against queued drafts. Drafts store raw english (no
        // english_normalized field), so we must recompute. This is O(D) where D is
        // typically small (< 20).
...
SanaeCompactStats SanaeProjectManager::Finalize() {
        sanae::PhaseTimer timer("sanae/profile/Finalize");
        ...
        timer.Mark("write_compact_cache");
        ...
        preserve_warning("project memory", [&] { RebuildMemory(); });
        timer.Mark("rebuild_memory");
        preserve_warning("repeat cache", [&] { RebuildRepeatCache(); });
        timer.Mark("rebuild_repeat_cache");
