...
wxString issue_detail(std::string const& value) {
        // value is already localized by SanaeProjectManager::TerminologyConsistencyIssues
        // using agi::format(_(...), ...). No post-hoc wxString::Replace needed.
        return to_wx(value);
}
...
class FinalReviewDialog final : public wxDialog {
        ...
        void Populate() {
                wxBusyCursor busy;  // visual feedback for potentially slow scans
...
bool ShowSanaeTerminologyEntryDialog(
        ...
        std::string russian)
{
        // Phase 2.8: TerminologyEntryPopover replaces TerminologyEntryDialog for the
        // simple "add new term" flow. The popover calls QueueTerminology directly.
        //
        // TerminologyEntryDialog is retained as a class because TermEditDialog
        // (in dialog_sanae_terminology.cpp) handles editing EXISTING terms with
        // server-conflict warnings — the popover does NOT have that parity yet.
        // When the popover gains edit/conflict-warning support, TermEditDialog
        // can be removed.
        auto *popover = new TerminologyEntryPopover(
...
bool ShowSanaeFinalReview(agi::Context *context, wxWindow *parent) {
        ...
        sanae::ux::modal_opened("final_review");
