...
if test x$git_version_str != x; then
  case "$git_version_str" in
    sanae-beta-*)
      beta_text=${git_version_str#sanae-beta-}
      if ! echo "$beta_text" | grep -Eq '^[0-9]+$'; then
        echo "invalid Sanae beta tag: $git_version_str" >&2
        exit 2
      fi
      sanae_beta_number=$(expr "$beta_text" + 0)
      beta_text=$(printf '%02d' "$sanae_beta_number")
      sanae_release_tag="sanae-beta-${beta_text}"
      sanae_version_str="sanae-v0.${sanae_beta_number} (beta)"
      sanae_product_str="Sanae — Aegisub v0.${sanae_beta_number} (beta)"
      git_version_str="Sanae-beta-${beta_text}"
      installer_version="3.5.${sanae_beta_number}"
      resource_version="3, 5, ${sanae_beta_number}"
      tagged_release=1
      ;;
    *)
      git_version_str="${git_version_str##v}"
      tagged_release=1
      if echo "$git_version_str" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+$'; then
...
#define SANAE_VERSION_STRING \"${sanae_version_str}\"
#define SANAE_PRODUCT_STRING \"${sanae_product_str}\"
#define SANAE_RELEASE_TAG \"${sanae_release_tag}\"
#define SANAE_BETA_NUMBER ${sanae_beta_number}"
